...
// ─── CRON ROUTE: AUTO-SCAN FOR STALE CHATS ──────────────────
// This route is triggered by Vercel Cron or a manual request.
app.get('/api/cron/scan', async (req, res) => {
  // Enhanced security for cron route (optional but recommended)
  ...
  try {
    ...
    for (const sub of activeUsers) {
      try {
        ...
        if (result.scheduled > 0) {
          ...
          await logInfo(sub.user_id, 'cron_scan', `Auto-scan: ${result.message}`);
        }
      } catch (userErr) {
        console.error(`  ❌ Cron error for user ${sub.user_id}:`, userErr.message);
        results.push({ userId: sub.user_id, error: userErr.message });
      }
    }
    ...
    res.status(500).json({ success: false, error: cronErr.message });
  }
});
...
// Cron logic moved to /api/cron/scan route for Vercel Compatibility
